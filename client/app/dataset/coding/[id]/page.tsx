    ...
    <>
      ...
      <div className="w-full flex h-full bg-gray-300 dark:bg-gray-800">
        ...
        <div className="flex-1 flex flex-col min-h-0 h-full overflow-hidden min-w-0 bg-transparent">
          ...
          {/* div3 */}
          <div
            className={"w-auto flex-1 min-h-0 overflow-y-auto px-1" + scrollbar}
          >
          ...
          <div className="p-1 items-center border-t-2 border-gray-400 flex flex-col gap-1">
            <div className="flex-1 flex flex-col lg:flex-row items-center justify-center gap-x-2 w-full lg:w-[70%]">
              <CodeInput
                placeholder="Add new code..."
                key={`${rows[keys[currentIndex]]?.id}:${
                  rows[keys[currentIndex]]?.code ?? "code"
                }`}
                defaultValues={rows[keys[currentIndex]]?.code ?? ""}
              ...
              <CodeInput
                ...
                defaultValues={rows[keys[currentIndex]]?.theme ?? ""}
                rowId={String(rows[keys[currentIndex]]?.id)}
                onSave={updateTheme}
              />
            </div>
